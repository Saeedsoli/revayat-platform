...
### ۱.1. ماهیت محصول (هم‌سو با فصل‌های ۱ و ۲ دامنه)
...
- **سیاست‌ها:**
  ...
  - **۱۳–۱۷ (Teen):**
    - دسترسی به محتوای آموزشی، تمرین‌ها، بازخورد AI و انسانی و خرید اشتراک/اعتبار مجاز است؛
    - **Critique Circles:**  
...
5. حلقه‌های نقد (Critique Circles) به عنوان فیچر تمایزدهندهٔ MVP:
   - حلقه‌های سه‌نفره،
   - Exercise-specific (هر حلقه برای یک تمرین مشخص در یک Path مشخص)،
   - جداسازی بر اساس زبان (`fa/en`) و بازهٔ سنی (`teen/adult`)،
   - نقد متقابل تمرین‌ها،
   - قفل مشارکت (N=2 نقد جدی، حداقل ۲۰۰ و حداکثر ۵۰۰۰ کاراکتر برای هر نقد)،
...
10. فاز ۱ خلاقیت (MVP-Level):
    ...
    - **Idea Playground (زمین بازی ایده‌ها) – مود Creative Clash به عنوان بازی تعاملی خلاقیت**:
      ...
      - امکان استفادهٔ عمومی (حتی برای مهمان) و ذخیرۀ جرقه‌ها برای کاربران لاگین‌کرده (در صورت پیاده‌سازی).
...
- **AI:** Claude 3.5 Sonnet / GPT‑4o از طریق Worker AI  
...
- `age_band` به‌صورت محاسبه‌ای از `users.date_of_birth` استخراج می‌شود:
  - ۱۳–۱۷ → `'teen'`,
  - ۱۸+ → `'adult'`.
...
- در فرم ثبت‌نام، فیلدهای زیر الزاماً جمع‌آوری می‌شوند:
  - `email` (یونیک، اجباری)،
...
- Backend:
  - پس از Auth، رکورد `users` را ایجاد/به‌روزرسانی می‌کند،
...
2. برای هر تمرین:
   ...
   - `submit` → `POST /submissions/{id}/submit`:
     - Validate (مالکیت، طول متن، محدودیت تعداد ارسال در روز – پیش‌فرض حداکثر ۱۰ ارسال/روز که در Config قابل تنظیم است)،
...
3. بررسی محدودیت حلقه بر حسب نوع کاربر، با تعریف «حلقۀ فعال برای کاربر»:
   ...
   - **حلقۀ فعال برای کاربر** یعنی:
     ...
     - و برای Submission خودش در آن حلقه `peer_feedback_unlocked = false` است (هنوز دو نقد لازم را ننوشته).
   ...
   - **کاربر دارای اشتراک فعال:**
     ...
     - اگر در دو حلقۀ فعال (با دو Path) حضور دارد و برای سومین حلقه درخواست می‌دهد:
       - Join مجاز است،
...
- منطق:
  ...
  - اگر count ≥ N (در MVP: N=2) → `peer_feedback_unlocked=true` برای Submissionهای user در آن حلقه و باز کردن دسترسی او به نقدهای دریافتی.
...
| ۶ | دارای اشتراک فعال | join در Path C (سومین حلقه، در حالی که دو حلقۀ قبلی هنوز برای او فعال‌اند) | موفق + `warning=true` |
...
- Backend:
  ...
  - رفتار دقیق طبق [فصل هفدهم دامنه](./product-domain-spec.md#chapter-17) و بخش ۴.۷ سند معماری است:
    - در حالت ناشناس‌سازی:
      ...
      - FKهای user در جداول جامعه‌محور به `NULL` ست می‌شوند؛
      - متن تمرین‌ها، نقدها و مشارکت‌ها بدون پیوند به هویت باقی می‌مانند.
    - در حالت حذف کامل:
...
- **کمینه‌سازی داده:**  
  فقط داده‌های لازم برای Auth، سیاست سنی/کشوری، یادگیری، پرداخت، و Analytics اختیاری جمع‌آوری می‌شود.

- **IP Anonymization:**  
  در Cloudflare Worker، IP کامل به `x.y.z.0` تبدیل می‌شود؛ Backend فقط این نسخهٔ ماسک‌شده را می‌بیند و ذخیره می‌کند.

- **Analytics Opt-in:**  
  پیش‌فرض غیرفعال؛ کاربر در تنظیمات (`/me/settings`) می‌تواند فعال/غیرفعال کند.  
  رویدادهای اختیاری شامل:
  - بازدید صفحات،
  - شروع/ذخیره/ارسال تمرین،
  - خوانده‌شدن بازخورد،
  - جست‌وجوی داخلی،
  - اسکن کدهای پاسخ‌گو (تجمیعی)،
  - وب‌ویتال‌ها،
  - ثبت لایک‌ها/واکنش‌ها به عنوان **رویداد تحلیلی** (دادهٔ عملیاتی فیچر جداگانه و همیشه ذخیره می‌شود، مطابق فصل ۱۶ دامنه و سند معماری).

- **Retention:**  
  - `events_behavior`: ۳۰ روز،  
  - `events_technical`: ۹۰ روز،  
  - داده‌های مالی: ۷ سال.

- **داده‌های جامعه‌محور (مثل Critique Circles):**
  - پیش‌فرض: ناشناس‌سازی مشارکت‌ها پس از حذف حساب، مگر درخواست حذف کامل.
  - در حالت ناشناس‌سازی، FKهای `user_id`/`author_id`/`reviewer_id` در جداولی مانند `circle_members`, `circle_submissions`, `peer_feedback` به `NULL` تبدیل می‌شوند؛
  - این سیاست در نسخهٔ ساده و نسخهٔ کامل سیاست داده، و در فصل هفدهم/هجدهم دامنه توضیح داده می‌شود.

- **گزارش شفافیت:**  
  هر ۳ ماه، گزارشی از:
  - تعداد حذف حساب،
  - نسبت ناشناس‌سازی/حذف کامل،
  - رخدادهای امنیتی،
  - آمار خطاها،
  - تأخیر منتورها،
  - تغییرات سیاست‌ها
...
- در MVP، یک مود اصلی پیاده می‌شود:
  ...
  - پس از پایان زمان، می‌تواند:
    - متن را کپی کند،
    - و اگر لاگین است، در صورت پیاده‌سازی، آن را به‌عنوان «جرقهٔ ایده» در پروفایل خود ذخیره کند.
  - این مود:
...
|:------|:-------|:-----------|:--------|
| احراز هویت (Email + Discord) | MVP | Supabase Auth | فیلدهای هویتی: ایمیل، نام مستعار، تاریخ تولد، کشور، زبان، جنسیت |
| مسیر «از ایده تا طرح کوتاه» | MVP | Learning Module | حداقل یک مسیر کامل |
| سیستم ارسال تمرین | MVP | Learning Module | draft/submit/autosave، محدودیت پیش‌فرض ۱۰ تمرین/روز (قابل تنظیم) |
| بازخورد AI سقراطی | MVP | AI Worker, LLM | مطابق فصل ششم دامنه |
| بازخورد منتور انسانی | MVP | Feedback Module | صف مشترک Pull-based |
| حلقه‌های نقد (Critique Circles) | MVP | Community Module | Exercise-specific, teen/adult, مطابق feature-spec |
| مدیریت ContentItems | MVP | Content Module | مرکز یادگیری |
| پرداخت زرین‌پال | MVP | Commerce Module | داخل ایران، تمام محصولات |
| پرداخت کریپتو (USDT/Polygon) | MVP | Crypto Worker | خارج از ایران، تمام محصولات (EPUB+NFT، اشتراک‌ها، اعتبار) |
...
| حذف حساب (دوحالته) | MVP | Compliance Module | ناشناس‌سازی/حذف کامل؛ قطع دسترسی آنلاین مجدد به EPUB |
| Skill Tree (آمار تجمیعی) | MVP | Analytics | بدون Leaderboard |
| داشبورد شفافیت و رشد فردی | MVP | Analytics | `/me/transparency` |
| Idea Playground – Creative Clash | MVP | Frontend + (اختیاری) Analytics | مود اصلی بازی خلاقیت، بدون نمره |
...
| مانیتورینگ کامل | MVP | Prometheus/Grafana/Loki/Sentry | self-hosted |
