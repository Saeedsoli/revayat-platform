...
۲. **ظرفیت حلقه:**
   ...
   - قیود منطقی:
     - `capacity` حداقل ۲ و حداکثر ۱۰؛ هر مقدار خارج از این بازه باید در لایهٔ اپ جلوگیری شود.
...
7. **تعریف «حلقۀ فعال برای کاربر» (Active Circle For User):**
   - برای اعمال محدودیت تعداد حلقه‌ها، «حلقۀ فعال برای کاربر» چنین تعریف می‌شود:
     ...
     - و برای Submission خودش در آن حلقه، `peer_feedback_unlocked = false` است؛
       یعنی هنوز تعداد نقدهای لازم (N) را برای دیگران ننوشته و نقدهای دیگران روی تمرین خودش unlock نشده است.
   - به‌محض این‌که کاربر حداقل N نقد معتبر بنویسد و `peer_feedback_unlocked` روی Submission خودش در آن حلقه `true` شود، آن حلقه از نظر محدودیت سهمیه‌ای دیگر «فعال» برای او به حساب نمی‌آید، حتی اگر وضعیت حلقه در جدول `critique_circles.status` همچنان `active` باشد.
...
8. **محدودیت تعداد حلقه‌های فعال (در کل سیستم):**
   ...
   - **کاربر دارای اشتراک فعال:**
     ...
     - اگر یک کاربر دارای اشتراک فعال در دو حلقۀ فعال (در دو Path مختلف) عضو باشد و برای سومی درخواست Join بدهد:
       ...
       - اما Backend باید در پاسخ، فلگ هشدار (مثلاً `warning: true, warningCode: 'MULTIPLE_ACTIVE_CIRCLES'`) برگرداند تا UI یک بنر غیرمسدودکننده نشان دهد («شما در ۳ حلقهٔ هم‌زمان فعال هستید»).
...
10. **قفل مشارکت (Participation Lock):**
    - کاربر تا زمانی که حداقل **N نقد** (در MVP، `N=2`) برای تمرین‌های دیگر اعضای حلقه ننوشته است:
      - نقدهای دریافت‌شده روی تمرین خودش را نمی‌بیند (`peer_feedback_unlocked=false`).
    - پس از ثبت N نقد معتبر:
      - نقدهای دریافتی روی تمرین او unlock می‌شوند (`peer_feedback_unlocked=true`).
    - **N در MVP عدد ثابت ۲ است** و از **متغیر محیطی** (مثلاً `CRITIQUE_REQUIRED_REVIEWS=2`) یا فایل Config خوانده می‌شود؛  
      نیازی به UI برای تغییر این مقدار در MVP نیست.
    - فقط نقدهایی شمرده می‌شوند که:
...
5. محدودیت تعداد حلقه:
   - منطبق با تعریف «حلقۀ فعال برای کاربر»:
     ...
     - اگر کاربر دارای اشتراک فعال در دو حلقۀ فعال (در Pathهای مختلف) است و برای سومی Join می‌دهد:
       - Join مجاز است؛
       - Backend در پاسخ، فلگ `warning: true, warningCode: 'MULTIPLE_ACTIVE_CIRCLES'` برمی‌گرداند.
...
2. فیلترکردن حلقه‌هایی که:
   - `critique_circles.status IN ('open','active')`  
     (در عمل برای کاربران Teen, Adult جداسازی از قبل در سطح ساختار صورت گرفته است).
...
- اگر `count >= N` (N=2):
  - برای تمام Submissionهای user در همان حلقه:
    - `peer_feedback_unlocked = TRUE`.
...
4. در `/[lang]/circles`:
   ...
   - اگر `status='active'`: نمایش کارت تمرین‌های دیگران (با دکمهٔ «نوشتن نقد») و تمرین خود کاربر (با قفل، تا زمان نوشتن ۲ نقد).
...
۱. **حلقه‌ای که هرگز به ظرفیت کامل نمی‌رسد:**
   - اگر حلقه‌ای مدت طولانی `open` بماند (مثلاً فقط ۲ نفر در آن باشند):
   ...
   - در فازهای بعد می‌توان حلقه‌های نیمه‌کامل را ادغام یا زمان‌بندی کرد.

۲. **ترک حلقه توسط کاربر (Leave Circle):**
   - در MVP فیچر Leave رسمی ندارد (برای ساده‌سازی).
   - خروج از حلقه در سطح داده تنها با مکانیسم‌های حذف حساب یا مداخلات Admin رخ می‌دهد.
   - اگر در آینده اضافه شود، باید اثر بر نقدهای داده‌شده/دریافت‌شده روشن بشود.
...
- `circle.all_reviews_done` – زمانی که همهٔ اعضا N نقد را تکمیل کرده‌اند.
